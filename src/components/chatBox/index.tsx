...
const Chatbox = ({
  ...
  const [messages, setMessages] = useState<
    { sender: { fullName: string }; content: string }[]
  ...
  useEffect(() => {
    ...
    if (currentChat && currentChat._id) handleFetchMessages();
  }, [currentChat]);
  ...
  const handleSendMessage = async () => {
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          sender: { fullName: loggedInUser.fullName },
          content: formattedMessage,
        },
      ]);
    } catch (error) {
    ...
    <div className="fixed inset-0 flex items-center justify-center z-50 backdrop-filter backdrop-blur-sm">
      <div className="absolute bg-cs-dark-gray w-[581px] h-[351px] border border-cs-gray border-[12px] p-2 px-4">
        <div className="flex justify-between">
          {!(selectedUser.fullName!.length > 0) &&
        ...
        {(currentChat || selectedUser.fullName) && (
          <div className="absolute bottom-1 w-full">
            <div className="flex items-center mt-2 mb-1">
              ...
              <img
                src={messageIcon}
                alt="Add Friend"
                className="cursor-pointer ml-2 mr-1 mt-1"
              />
              <button
                onClick={() => handleFormat("bold")}
                className="mr-1 p-1 font-bold text-cs-soft-gray-two"
              >
                B
              </button>
              <button
                onClick={() => handleFormat("italic")}
                className="mr-1 p-1 italic text-cs-soft-gray-two"
              >
                I
              </button>
              <button
                onClick={() => handleFormat("underline")}
                className="mr-1 mt-1 p-1 text-cs-soft-gray-two"
              >
                <p>U</p>
              ...
              <img
                src={closeIcon}
                alt="Add Friend"
                className="cursor-pointer mt-1 w-[20px] h-[20px]"
              />
            </div>
            <div
              ref={chatboxRef}
